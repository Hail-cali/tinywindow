...
static void test_create_destroy(void) {
    nf_timing_cms_t *cms = nf_tcms_create(5, 1000, 12, 300000);
    assert(cms != NULL);
    printf("  memory usage: %zu bytes\n", nf_tcms_memory_usage(cms));
    nf_tcms_destroy(cms);

    /* Invalid params */
    assert(nf_tcms_create(0, 1000, 12, 300000) == NULL);
    assert(nf_tcms_create(5, 0, 12, 300000) == NULL);
    assert(nf_tcms_create(5, 1000, 0, 300000) == NULL);
    assert(nf_tcms_create(5, 1000, 12, 0) == NULL);
...
static void test_basic_count(void) {
    ...
    nf_timing_cms_t *cms = nf_tcms_create(5, 10000, 12, 300000);
    assert(cms != NULL);
    ...
    for (int i = 0; i < 5; i++) {
        assert(nf_tcms_record(cms, "user:abc", now) == NF_OK);
    }

    /* Count within full window (12 slots * 300s = 3600s = 1 hour) */
    uint64_t count = nf_tcms_count(cms, "user:abc", 3600000, now);
    ...
    uint64_t count_unknown = nf_tcms_count(cms, "user:unknown", 3600000, now);
...
static void test_multi_key_accuracy(void) {
    ...
    nf_timing_cms_t *cms = nf_tcms_create(5, 50000, 1, 3600000);
    assert(cms != NULL);
    ...
    for (int i = 0; i < 100; i++) {
        nf_tcms_record(cms, "hot_key_0", now);
    }
    for (int i = 0; i < 50; i++) {
        nf_tcms_record(cms, "hot_key_1", now);
    }
    ...
    uint64_t c0 = nf_tcms_count(cms, "hot_key_0", 3600000, now);
    uint64_t c1 = nf_tcms_count(cms, "hot_key_1", 3600000, now);
...
static void test_reset(void) {
    nf_timing_cms_t *cms = nf_tcms_create(4, 1000, 6, 10000);
    assert(cms != NULL);
    ...
    assert(nf_tcms_count(cms, "key", 60000, now) >= 10);

    nf_tcms_reset(cms);
    assert(nf_tcms_count(cms, "key", 60000, now) == 0);
